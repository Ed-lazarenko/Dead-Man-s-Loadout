document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ───────────────── Сброс файла scores.json ───────────────── */
document.getElementById('reset-scores-btn')?.addEventListener('click', async () => {
  const ok = confirm('Очистить balances?\nФайл scores.json будет обнулён.');
  if (!ok) return;

  try {
    /* записываем ОДИН пустой объект {} */
    await fetch('/api/scores', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: '{}'
    });
    alert('Файл scores.json очищен.');
  } catch (e) {
    alert('Не удалось очистить файл. Проверь сервер.');
    console.error(e);
  }
});
